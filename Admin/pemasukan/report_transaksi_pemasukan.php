...
if ((isset($_GET['submit']) || isset($_GET['export'])) && !empty($_GET['from']) && !empty($_GET['to'])) {
    // Format tanggal dengan rentang waktu penuh
    $from = date('Y-m-d 00:00:00', strtotime($_GET['from']));
    $to = date('Y-m-d 23:59:59', strtotime($_GET['to']));

    // Query untuk mengambil data transaksi
    $total = mysqli_query($conn, "SELECT Tanggal_transaksi, Harga FROM transaksi WHERE Tanggal_transaksi BETWEEN '$from' AND '$to' ORDER BY Tanggal_transaksi");
    
    while ($row_total = mysqli_fetch_assoc($total)) {
        $data[] = $row_total["Tanggal_transaksi"];
        $data_harga[] = $row_total["Harga"];
        $jumlah_pelanggan++;
        $jumlah_pendapatan += $row_total['Harga'];
    }

    // Jika tombol export diklik
...
<body class="bg-light">
    ...
    <script>
        ...
        new Chart(ctx, {
            type: 'bar',
            data: {
                labels: <?= json_encode($data); ?>, // Hanya tanggal
                datasets: [{
                    label: 'Total',
